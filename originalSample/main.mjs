 ...
 */
async function requestLocalSearchAPI(query) {
  const response = await axios({
    url: 'https://knishina-eval-test.apigee.net/yahoo_map/search/local/V1/localSearch',
    params: {
      query,
      output: 'json',
    },
...
async function main() {
  ...
  document.getElementById('form').onsubmit = () => {
    ...
    // ジオコーダAPIの検索結果を反映させる
    (async () => {
      const geoFeatures = await requestLocalSearchAPI(query);
      const resultText = document.getElementById('result-text');
      if (geoFeatures.length > 0) {
        const [first] = geoFeatures;
        const lct = parseCoordinates(first.Geometry.Coordinates);
        const [station] = first.Property.Station;
        resultText.textContent = `場所: ${first.Name}, 緯度: ${lct.lat}, 経度: ${lct.lng}, 最寄駅: ${station.Name}`;
        // 地図を移動してラベル追加
        drawMap(map, lct);
        // const label = new Y.Label(new Y.LatLng(lct.lat, lct.lng), first.Name);
        const label = new Y.Label(new Y.LatLng(lct.lat, lct.lng), `緯度: ${lct.lat}, 経度: ${lct.lng}, 最寄駅: ${station.Name}`);
